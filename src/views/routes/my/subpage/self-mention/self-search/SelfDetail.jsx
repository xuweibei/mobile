...
class ReDetail extends BaseComponent {
    constructor(props) {
        ...
        this.state = {
            ...
            height: document.documentElement.clientHeight - (window.isWX ? 0.75 : window.rem * 1.08),
            propsData: props
        };
    }
    ...
    getList = (drawCircle = false) => {
        ...
        const {pageSize, pageCount, page, pageList} = this.state;
        this.fetch(urlCfg.mallSelfOrder, {data: {page, pagesize: pageSize, pageCount, status: 0, key: decodeURI(getUrlParam('keywords', encodeURI(this.props.location.search)))}}, drawCircle)
            .subscribe(res => {
                if (res && res.status === 0) {
                    //判断是否为最后一页
    ...
    render() {
        const {pageList, refreshing, isLoading, hasMore, canStatus, navColor, height} = this.state;
        //每行渲染样式
        const row = item => (
        ...
        return (
            <div data-component="Self-mention" data-role="page" className="self-order">
                <AppNavBar title="线下订单" backgroundColor={navColor} redBackground color={navColorR}/>
