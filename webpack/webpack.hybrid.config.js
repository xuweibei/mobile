...
const entry = require('./entry/entry.conf');
...
module.exports = (env = {}) => {
    ...
    // 多页面入口
    const newEntry = {}, newHtml = [];
    let count = 0;
    for (let name in entry) {
        console.log(entry[name])
        newEntry[name] = ['babel-polyfill', entry[name][0]];
        newHtml[count++] = new HtmlWebpackPlugin({
            title: entry[name][1],
            template: path.resolve(__dirname, ENTRYDIR + '/index-hybrid.ejs'),
            filename: `${entry[name][1]}.html`,
            inject: false,
            hash: true,    // 打包文件添加hash
            env: ENV.path(),
            chunks: ['manifest','vendor',entry[name][1]]
        })
    }
            ...
            rules: [
                ...
                /* Rules for js|jsx */
                {
                    ...
                    rules: [
                        /* eslint */
                        {
                            include: [path.resolve(__dirname, ENTRYDIR), path.resolve(__dirname, MULTI)],
                            enforce: 'pre',
                            use: [{
