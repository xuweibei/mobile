...
(function (global, factory) {
    ...
    let LongPrototype = Long.prototype;
    ...
    LongPrototype.getNumBitsAbs = function getNumBitsAbs() {
        ...
        for (var bit = 31; bit > 0; bit--) {if ((val & (1 << bit)) != 0)
                break;}
 ...
 */
let msgCache = {};
const webim = { // namespace object webim
...
(function (webim) {
    ...
    const longPollingPackageTooLargeErrorCode = 10018;
    ...
    const maxApiReportItemCount = 20; //一次最多上报条数
            ...
            for (const a in json) {
                if (keyMap[a]) {
                    ...
                    if (json[keyMap[a]] instanceof Array) {
                        const len = json[keyMap[a]].length;
                        for (let i = 0; i < len; i++) {
    ...
    //发起ajax请求
    const ajaxRequest = function (meth, url, req, timeout, content_type, isLongPolling, cbOk, cbErr) {
    ...
    //发起ajax请求（json格式数据）
    const ajaxRequestJson = function (meth, url, req, timeout, content_type, isLongPolling, cbOk, cbErr) {
        ajaxRequest(meth, url, JSON.stringify(req), timeout, content_type, isLongPolling, (resp) => {
            let json = null;
    ...
    var proto_accesslayer = function (callback) {
        ConnManager.apiCall(SRV_NAME.WEB_IM, 'accesslayer', {}, (data) => {
...
image;
    ...
    //获取群组未决列表
    const proto_getPendencyGroup = function (options, cbOk, cbErr) {
        if (!checkLogin(cbErr, true)) return;
    ...
    //群组未决已经上报
    const proto_getPendencyGroupRead = function (options, cbOk, cbErr) {
        if (!checkLogin(cbErr, true)) return;
    ...
    //处理被邀请进群申请(同意或拒绝)
    const proto_handleInviteJoinGroupRequest = function (options, cbOk, cbErr) {
        if (!checkLogin(cbErr, true)) return;
    ...
    //删除会话
    const proto_deleteChat = function (options, cbOk, cbErr) {
        if (!checkLogin(cbErr, true)) return;
    ...
    //好友申请已读上报
    const proto_getPendencyReport = function (options, cbOk, cbErr) {
        if (!checkLogin(cbErr, true)) return;
    ...
    var ConnManager = lowerBR == false ? new function () {
        ...
        this.apiCall = function (type, cmd, data, cbOk, cbErr, timeout, isLongPolling) {
            ...
            if (url == false) return;
            //发起ajax请求
            const content_type = data.content_type ? data.content_type : 'application/x-www-form-urlencoded';
            ajaxRequestJson('POST', url, data, timeout, content_type, isLongPolling, (resp) => {
                let errorCode = null,
                    tempErrorInfo = '';
                if (cmd == 'pic_up') {
                    data.Slice_Data = '';
    ...
    const calcUniqId = function (num1, num2) {
        const str1 = parseInt(num1).toString(2) + '00000000000000000000000000000000';
        const str2 = parseInt(num2).toString(2);
        ...
        const longVal = new Long(long.low, long.high, true);
        return longVal.toString();
    };
    // class Msg
    const Msg = function (sess, isSend, seq, random, time, fromAccount, subType, fromAccountNick, fromAccountHeadurl) {
        this.sess = sess;
    ...
    Msg.Elem.Images.prototype.getImage = function (type) {
        ...
        let img = null;
        this.ImageInfoArray.forEach((image) => {
    ...
    Msg.Elem.File = function (uuid, name, size, senderId, receiverId, downFlag, chatType, url) {
        ...
        if (downFlag == 2 && url != null) {
            this.downUrl = url;
        } else if (downFlag !== undefined && this.busiId !== undefined) {
    ...
    Msg.Elem.GroupTip = function (opType, opUserId, groupId, groupName, userIdList, userinfo) {
        ...
        this.userIdList = userIdList || []; //被操作的用户id列表
        ...
        this.userinfo = userinfo || []; //被操作的用户信息列表列表
    };
    ...
    var MsgStore = new function () {
        ...
        const checkDupMsg = function (msg) {
            ...
            const second_key = [msg.isSend ? '1' : '0', msg.seq, msg.random].join('');
    ...
    var MsgManager = new function () {
        ...
        const isLongPollingRequesting = false; //是否在长轮询ing
        ...
        //我的群当前最大的seq
        const myGroupMaxSeqs = {}; //用于补拉丢失的群消息
        ...
        var handlerOrdinaryAndTipC2cMsgs = function (eventType, groupMsgArray) {
            const groupMsgMap = {}; //保存收到的C2c消息信息（群号，最小，最大消息seq，消息列表）
            let new_group_msgs = [];
        ...
        const handlerOrdinaryAndTipGroupMsgs = function (eventType, groupMsgArray) {
            const groupMsgMap = {}; //保存收到的群消息信息（群号，最小，最大消息seq，消息列表）
            let new_group_msgs = [];
            const minGroupMsgSeq = 99999999;
            const maxGroupMsgSeq = -1;
            for (const j in groupMsgArray) {
        ...
        const handlerGroupSystemMsgs = function (groupSystemMsgs, isNeedValidRepeatMsg) {
            for (const k in groupSystemMsgs) {
                ...
                if (isNeedValidRepeatMsg) {
                    ...
                    const key = groupTip.ToGroupId + '_' + reportType + '_' + groupReportTypeMsg.Operator_Account + '_' + groupTip.ClientSeq;
                ...
                if (isNeedValidRepeatMsg) {
                    ...
                    if (onGroupSystemNotifyCallbacks[reportType]) {
                        if (reportType == GROUP_SYSTEM_TYPE.READED) {
                            const arr = notify.groupReportTypeMsg.GroupReadInfoArray;
        ...
        */
            ...
            case C2C_EVENT_SUB_TYPE.READED:
                ...
                if (notify.ReadC2cMsgNotify && notify.ReadC2cMsgNotify.UinPairReadArray && onC2cEventCallbacks[subType]) {
                    for (let i = 0, l = notify.ReadC2cMsgNotify.UinPairReadArray.length; i < l; i++) {
                        const item = notify.ReadC2cMsgNotify.UinPairReadArray[i];
        ...
        var handlerOrdinaryAndTipC2cMsgs = function (eventType, C2cMsgArray) {
            ...
            for (const i in msgInfos) {
                ...
                var isSendMsg,
                    id;
                const headUrl = msgInfo.From_AccountHeadurl || '';
                ...
                const msg = new Msg(sess, isSendMsg, msgInfo.MsgSeq, msgInfo.MsgRandom, msgInfo.MsgTimeStamp, msgInfo.From_Account, C2C_MSG_SUB_TYPE.COMMON, msgInfo.From_AccountNick, headUrl);
                let msgBody = null;
                let msgContent = null;
                let msgType = null;
        ...
        this.syncMsgs = function (cbOk, cbErr) {
            ...
            proto_getMsgs(MsgStore.cookie, MsgStore.syncFlag, (resp) => {
                ...
                for (const i in msgInfos) {
                    ...
                    const msg = new Msg(sess, isSendMsg, msgInfo.MsgSeq, msgInfo.MsgRandom, msgInfo.MsgTimeStamp, msgInfo.From_Account, C2C_MSG_SUB_TYPE.COMMON, msgInfo.From_AccountNick, msgInfo.From_AccountHeadurl);
                    let msgBody = null;
                    let msgContent = null;
                    let msgType = null;
                    for (const mi in msgInfo.MsgBody) {
        ...
        this.syncGroupMsgs = function (options, cbOk, cbErr) {
            ...
            proto_getGroupMsgs(opts, (resp) => {
                const notifyInfo = [];
                const group_id = resp.GroupId; //返回的群id
                const msgInfos = resp.RspMsgList; //返回的消息列表
        ...
        var handlerGroupMsg = function (msgInfo, isSyncGroupMsgs, isAddMsgFlag, isFinished) {
            ...
            const msg = new Msg(sess, isSendMsg, msgInfo.MsgSeq, msgInfo.MsgRandom, msgInfo.MsgTimeStamp, msgInfo.From_Account, subType, fromAccountNick, fromAccountHeadurl);
            let msgBody = null;
            let msgContent = null;
            let msgType = null;
            for (const mi in msgInfo.MsgBody) {
    ...
    const FileUploader = new function () {
        ...
        const getFileMD5 = function (file, cbOk, cbErr) {
            ...
            const chunkSize = 2 * 1024 * 1024; //分块大小，2M
        ...
        this.submitUploadFileForm = function (options, cbOk, cbErr) {
            ...
            const uploadApiUrl = 'https://pic.tim.qq.com/v4/openpic/' + cmdName + '?tinyid=' + ctx.tinyid + '&a2=' + ctx.a2 + '&sdkappid=' + ctx.sdkAppID + '&accounttype=' + ctx.accountType + '&contenttype=http';
